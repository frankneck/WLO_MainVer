...
public partial class FirstPersonPlayerInputsSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
            ...
            .WithAll<GhostOwnerIsLocal>())
        {
            ...
            if (UIController.Instance.HasStateFlag(GameUIState.Inventory) || 
                UIController.Instance.HasStateFlag(GameUIState.Menu))
            {
                inputActions.Disable();
            }
            else
...
public partial struct FirstPersonPlayerVariableStepControlSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        FirstPersonPlayerVariableStepControlJob job = new FirstPersonPlayerVariableStepControlJob
        {
            ...
            CharacterInteractionControlLookup = SystemAPI.GetComponentLookup<CharacterInteractionControl>(false),
        };
...
public partial struct FirstPersonPlayerVariableStepControlJob : IJobEntity
{
    ...
    public ComponentLookup<CharacterInteractionControl> CharacterInteractionControlLookup;
        ...
        in FirstPersonPlayer player)
    {
        ...
        // Character interact
        if (CharacterInteractionControlLookup.HasComponent(player.ControlledCharacter))
        {
            CharacterInteractionControl characterInteractControl = CharacterInteractionControlLookup[player.ControlledCharacter];
            
            characterInteractControl.Interact = playerCommands.InteractPressed.IsSet;

            CharacterInteractionControlLookup[player.ControlledCharacter] = characterInteractControl;
        }
    }
}
