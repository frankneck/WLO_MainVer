        ...
        foreach (var evt in connectionEventsForClient)
        {
            switch (evt.State)
            {
                ...
                case ConnectionState.State.Connected:
                    ...
                    AsyncOperation asyncGameplayOperation = SceneManager.LoadSceneAsync(1, LoadSceneMode.Single);            
                    AsyncOperation asyncPresentationOperation = SceneManager.LoadSceneAsync(2, LoadSceneMode.Additive);
                    asyncGameplayOperation.allowSceneActivation = true;
                    asyncPresentationOperation.allowSceneActivation = true;
                    break;
