using Unity.Burst;
using Unity.Collections;
using Unity.Entities;
using Unity.NetCode;

[BurstCompile]
[WorldSystemFilter(WorldSystemFilterFlags.ServerSimulation)]
public partial struct ServerInitSystem : ISystem
{
    [BurstCompile]
    public void OnStart(ref SystemState state)
    {
        var queryBuilder = new EntityQueryBuilder(Allocator.Temp).WithAll<PlayerInitRequest, ReceiveRpcCommandRequest>();
        state.RequireForUpdate(state.GetEntityQuery(queryBuilder));
    }

    [BurstCompile]
    public void OnUpdate(ref SystemState state)
    {
        if (SystemAPI.HasSingleton<GhostPrefabs>())
        {
            EntityCommandBuffer ecb = new EntityCommandBuffer(Allocator.Temp);
            
            foreach (var (recieveRPC, initRequest, entity) in SystemAPI.Query<ReceiveRpcCommandRequest, PlayerInitRequest>().WithEntityAccess())
            {
                var requestedTeam = initRequest.TeamValue;
                var playerName = initRequest.PlayerName;

                TeamType team;
                if (requestedTeam == TeamRequest.Auto)
                {
                    team = GetAutoTeam(ref state);
                }
                else
                {
                    team = requestedTeam.ToTeamType();
                }

                ecb.AddComponent(recieveRPC.SourceConnection, new ServerPlayerInitRequest { 
                    PlayerName = playerName, 
                    TeamValue =  team,
                });

                ecb.DestroyEntity(entity);
                UnityEngine.Debug.Log("[Server] Player has been initialized.");
            }
            ecb.Playback(state.EntityManager);
            ecb.Dispose();
        }
    }

    private TeamType GetAutoTeam(ref SystemState state)
    {
        // TODO: Assigning auto team
        return TeamType.Blue;
    }
}